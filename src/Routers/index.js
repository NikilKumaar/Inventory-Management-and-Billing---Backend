    ...
    if (checkUser.length === 0) {
      ...
      if (validatingPassword) {
        ...
        if (checkUser[0].status === "Active") {
          ...
          return res
            .status(200)
            .json({ message: "login success", data: checkUser[0], token });
        } else {
          ...
          return res
            .status(200)
            .json({
              message: "Account is Not Active, Activation Link sent to mail",
            });
        }
      } else {
      ...
      checkingData.find((value, index) => {
        if (req.body.productName === value.productName) {
          checkingData[index] = {
            ...
            productQuantity: req.body.productQuantity + value.productQuantity,
            productPrice: req.body.productPrice,
          };
          return true;
        }
...
router.post("/editProduct", isAuthorized, async (req, res) => {
  try {
    ...
    const filteredData = data.filter(
      (value, index) => value.productName != req.body.oldProductName
    );
...
router.post("/deleteProduct", isAuthorized, async (req, res) => {
  try {
    ...
    const filteredData = data.filter(
      (value, index) => value.productName != req.body.productName
    );
      ...
      originalData.find((value1, index1) => {
        if (value1.productName === value.description) {
          originalData[index1] = {
            ...
            productQuantity: value1.productQuantity - value.quantity,
